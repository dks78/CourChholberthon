def conversion_unité():
    
    demander_user = input("Souhaite vous convertir des pouces vers cm ou cm vers pouces\n")
    if demander_user == "pouces vers cm":
        pouces = float(input("Entrez les pouces : "))  # Utilisation de float pour permettre des valeurs décimales
        cm = round(pouces * 2.54,2)
        print(f"{pouces} pouces équivaut à {cm} centimètres.")
    elif demander_user == "cm vers pouces":
        cm = float(input("Entrez les centimètres : "))  # Utilisation de float pour permettre des valeurs décimales
        pouces = round(cm / 2.54,2)
        print(f"{cm} centimètres équivaut {round(pouces)} pouces.")
    else:
...
#coorection 
# u pour unité de sortie
# POUCE pour la valeur d'une pouce
